...
CHAY NHANH — chia theo LOAI CHI PHI (23/08/2026, bo do dang mat ~6 phut):
    python scratchpad/check_quiz_bank.py --static   # [1][2][3][5] cau truc, ~20s
    python scratchpad/check_quiz_bank.py --urls     # [4] 82 URL tra 200 (cham nhat)
    python scratchpad/check_quiz_bank.py --play     # [6][7][8][9] choi that
    python scratchpad/check_quiz_bank.py            # CHAY DU — dung o cong push

⚠️⚠️ CHAY DU TRUOC MOI LAN PUSH, khong co ngoai le (quy tac 11 muc 6). Ba nhom tren
   la de sua nhanh trong luc lam, KHONG phai de thay the mot luot chay du: bang
   "sua gi chay bo nao" chi dung khi phan doan ve pham vi la dung, ma chinh lich su
   file nay ghi nhieu lan phan doan do SAI.

⚠️ Muc [4] CACHE ket qua 200 theo NGAY (`scratchpad/_urlcache/<ngay>.txt`).
   Chi cache URL tra 200; URL hong thi luot sau kiem lai ngay. `--fresh` bo cache.
   Bo do IN RA so URL lay tu cache — dung tin mot bo do im lang ve chuyen do.
"""
import math
import os
import sys
...
# ── chon nhom muc de chay (xem docstring) ──
ARGS = set(a.lstrip("-") for a in sys.argv[1:])
FRESH = "fresh" in ARGS
NHOM = {"static": {1, 2, 3, 5}, "urls": {4}, "play": {6, 7, 8, 9}}
_chon = set()
for k, v in NHOM.items():
    if k in ARGS:
        _chon |= v
DU = not _chon                      # khong chon gi = chay DU (cong push)
CHAY = set(range(1, 10)) if DU else _chon


def want(*muc):
    """Muc nay co nam trong nhom da chon khong."""
    return any(m in CHAY for m in muc)


# ── cache URL theo NGAY: chi nho URL TRA 200, het ngay la kiem lai ──
_CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_urlcache")


def _cache_path():
    import datetime
    return os.path.join(_CACHE_DIR, datetime.date.today().isoformat() + ".txt")


def url_cache_doc():
    """Tap URL da tra 200 TRONG NGAY. Rong neu --fresh hoac chua co file."""
    if FRESH:
        return set()
    p = _cache_path()
...
def url_cache_ghi(urls):
    """Ghi them cac URL vua tra 200. Hong thi bo qua — cache khong duoc lam vo bo do."""
    if not urls:
        return
    try:
        os.makedirs(_CACHE_DIR, exist_ok=True)
        cu = url_cache_doc()
        with open(_cache_path(), "a", encoding="utf-8", newline="\n") as f:
            for u in sorted(set(urls) - cu):
                f.write(u + "\n")
    except Exception as e:
        print("  [!] khong ghi duoc cache URL: %s" % str(e)[:60])
...
def main():
    with sync_playwright() as p:
        ...
        # ⚠️⚠️ NAP BANK LA DIEU KIEN TIEN QUYET, KHONG PHAI MOT PHEP KIEM — nen no
        #    nam NGOAI moi `if want(...)`. Ban dau toi boc no trong `if want(1)`, va
        #    `--urls` chay mot minh lien vo voi `UnboundLocalError: bank` (muc [2][3]
        #    [4][5] deu doc `bank`). Chia nho mot bo do thi phai tach phan DUNG CHUNG
        #    ra truoc, khong thi cac nhom chi chay duoc khi di kem nhom dau.
        ...
        if want(4):
            ...
            OK_HOSTS = ("https://science.nasa.gov/", "https://spaceplace.nasa.gov/",
                        ...
                        "https://www.esa.int/",
                        # ⚠️ NOI MOT TEN MIEN, 23/08/2026: `news.mit.edu` la
                        #    kenh tin chinh thuc cua MIT, va MIT von da la nguon
                        #    tin cay cua du an (`www.media.mit.edu` da o tren,
                        #    `wiki/` dan MIT tu Dot 1). Bai `art-ai-already-around-
                        #    you` dan dung trang nay va khong co trang NASA nao
                        #    noi ve AI trong DOI SONG. Khong mo cua cho URL bat ky.
                        "https://news.mit.edu/",
                        # ⚠️ NOI MOT TEN MIEN, 23/08/2026: chu du an chot
                        #    *"NASA ko chuyen ve AI"*. `ai4k12.org` la khung day AI
                        #    o K-12 cua My, "jointly sponsored by AAAI and CSTA", NSF
                        #    tai tro (DRL-1846073) — to chuc hoc thuat, KHONG ban san
                        #    pham. ⚠️ CO Y khong nhan OpenAI/Google/IBM: du an da mot lan
                        #    phai go `ibm.com/quantum` khoi kho bai doc vi dung ly do do.
                        "https://ai4k12.org/")
            ...
            #    cache · va IN RA so URL lay tu cache (mot bo do im lang ve chuyen no
            #    khong thuc su goi mang la mot bo do noi sai ve dieu no bao dam).
            _da200 = url_cache_doc()
            _moi200, _tu_cache = [], 0
            for url in sorted(srcs):
                ...
                if code == 200:
                    _moi200.append(url)
            url_cache_ghi(_moi200)
            print("      %d URL lay tu cache trong ngay · %d URL vua goi mang%s"
                  % (_tu_cache, len(srcs) - _tu_cache, "  (--fresh)" if FRESH else ""))
